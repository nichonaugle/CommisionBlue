...
derived_client_key = HKDF(
    algorithm=hashes.SHA256(),
    length=32,
    salt=None,
    info=b'handshake data'
).derive(client_shared_key)
aesgcm = AESGCM(key=derived_client_key) #shared key is 256 bit size #should we still use a KDF on the shared_key?
...
derived_server_key = HKDF(
    algorithm=hashes.SHA256(),
    length=32,
    salt=None,
    info=b'handshake data'
).derive(server_shared_key)
server_aesgcm = AESGCM(derived_server_key)
